...
$$ 150 = t(t + 1),$$
$$ 0 = t^2 + t - 150.$$
If Ax^2 + Bx + C = 0, then $x = \frac {- Bx \pm \sqrt{B^2 - 4AC}} {2A}.$
Plugging in, we get the critical points of x:
```{r}
(-1 + sqrt((1^2) - 4 * 1 * - 150)) / (2 * 1)
(-1 - sqrt(1^2 - (4 * 1) * (- 150))) / (2 * 1)
```
If t standas for number of days, the negative critical point doesn't really help us. But does 11.75 represent a local or global maximum? To the left:
```{r}
(30 / (10.76 + 1)) - (10.76 / 5)
```
Positive. \newline
...
(30 / (12.76 + 1)) - (12.76 / 5)
```
Negative. Therefore, using th efirst derivative test, 11.76 is a maximum. All we have left to compare that to is 0 plugged into the original function:
```{r}
30 * log(0 + 1) - (0^2 / 10)
```
