...
The model formula is $\text{logit}(\hat \pi) = `r round((coef(mbti_mod)[("(Intercept)"),1]),2)` `r round((coef(mbti_mod)[("ExtIntI"),1]),2)` `r round((coef(mbti_mod)[("SensIntuitS"),1]),2)` + `r round((coef(mbti_mod)[("ThinkFeelT"),1]),2)` + `r round((coef(mbti_mod)[("JudPerP"),1]),2)`$. The intercept is the ENFJ type; that is, when ExtInt = E, SensIntuit = N, ThinkFeel = F, and JudPer = J.
